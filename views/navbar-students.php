<nav class="navbar navbar-expand-lg navbar-light  bg-white">
    <a class="navbar-brand" href="#"></a>
    <button class="navbar-toggler d-lg-none" type="button" data-bs-toggle="collapse" data-bs-target="#collapsibleNavId" aria-controls="collapsibleNavId" aria-expanded="false" aria-label="Toggle navigation"></button>
    <div class="collapse navbar-collapse" id="collapsibleNavId">
        <ul class="navbar-nav me-auto mt-2 mt-lg-0">
            <li class="nav-item active">
                <a class="nav-link" href="students_dashboard.php"><img src="../images/CTU.png" alt="" height="50" width="50" srcset="">Student's Rating System For Teachers</a>
            </li>

        </ul>
        <form class="d-flex my-2 my-lg-0">
            <ul class="navbar-nav">
                
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="dropdownId" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Options <i class="fa fa-plus-circle" aria-hidden="true"></i> </i></a>
                    <div class="dropdown-menu" aria-labelledby="dropdownId">
                        <!-- <a class="dropdown-item" href="manage-college.php">Colleges <i class="fa fa-building float-end" aria-hidden="true"></i> </a>
                        <a class="dropdown-item" href="manage-evaluation.php">Evaluation <i class="fas fa-sticky-note float-end "></i> </a>
                        <a class="dropdown-item" href="manage-reports.php">Reports <i class="fa fa-line-chart float-end" aria-hidden="true"></i> </a>
                        <a class="dropdown-item" href="admin-users.php">Users <i class="fa fa-user float-end" aria-hidden="true"></i></a> -->
                        <a class="dropdown-item" href="logout.php?student=true">Logout <i class="fas fa-exclamation-circle    "></i>  </a>
                    </div>
                </li>
                <!-- <li class="nav-item">
                    <a class="nav-link" href="#">Active Reports <i class="fa fa-line-chart" aria-hidden="true"></i> </a>
                </li> -->
            </ul>
        </form>
    </div>
</nav>